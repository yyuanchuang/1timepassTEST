...
const Layout: React.FC<{ user: User | null; onLogout: () => void; children: React.ReactNode }> = ({ user, onLogout, children }) => {
  ...
  const getBadge = (path: string) => {
    if (path === '/admin' && counts.adminCount > 0) {
      return <span className="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold leading-none text-white bg-red-600 rounded-full">{counts.adminCount}</span>;
    }
    if (path === '/summary' && counts.workerCount > 0) {
      return <span className="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold leading-none text-white bg-red-600 rounded-full">{counts.workerCount}</span>;
    }
    return null;
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              <button onClick={onLogout} className="p-2 rounded-full hover:bg-brand-700 transition-colors" title="登出">
                <LogOut className="h-5 w-5" />
              </button>
            </div>
            
            <div className="md:hidden">
              <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 rounded-md text-brand-200 hover:text-white hover:bg-brand-700">
          ...
          <div className="md:hidden bg-brand-800">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {NAV_ITEMS.filter(item => item.roles.includes(user.role)).map(item => (
                <Link
                  ...
                  className={`block px-3 py-2 rounded-md text-base font-medium flex justify-between items-center ${
                    location.pathname === item.path ? 'bg-brand-900 text-white' : 'text-brand-100 hover:bg-brand-700'
                  }`}
                >
                  {item.label}
                  {getBadge(item.path)}
              ...
              <div className="border-t border-brand-700 mt-4 pt-4 px-3 flex items-center justify-between text-brand-100">
                ...
                <button onClick={onLogout} className="text-sm font-medium hover:text-white">登出</button>
              </div>
            </div>
          </div>
        )}
